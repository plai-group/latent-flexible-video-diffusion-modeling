...
export COLLECT_OUTPUT_DIR="plaicraft/comparison_online_${EPOCH}"
...
FVD_BATCH_SIZE=10
N_VISUALIZE=4
EVAL_DATASET_CONFIG='chunked'
LOWER_FRAME_RANGE=$EPOCH
...
RESULT_PATH=results/${WANDB_ID}/${MODEL_NAME}_${SAMPLER}/${SAMPLE_SCHEME}_${MAX_FRAMES}_${VID_LENGTH}_${N_START_FRAMES}_${EVAL_DATASET_CONFIG}_${LOWER_FRAME_RANGE}_${UPPER_FRAME_RANGE}_${STREAM_TYPE}
COLLECT_PREFIX=${MODEL_NAME}_${SAMPLER}/${SAMPLE_SCHEME}_${MAX_FRAMES}_${VID_LENGTH}_${N_START_FRAMES}_${EVAL_DATASET_CONFIG}_${LOWER_FRAME_RANGE}_${UPPER_FRAME_RANGE}_${STREAM_TYPE}

python scripts/video_sample.py $CHECKPOINT_PATH --T=$VID_LENGTH --stop_index=$NUM_GENERATIONS --num_sampled_videos=$NUM_GENERATIONS --max_frames=$MAX_FRAMES --n_obs=$N_START_FRAMES --sampling_scheme=$SAMPLE_SCHEME --batch_size=$SAMPLE_BATCH_SIZE --eval_on_train=$EVAL_ON_TRAIN --sampler=$SAMPLER --eval_dataset_config=$EVAL_DATASET_CONFIG --decode_chunk_size=$DECODE_CHUNK_SIZE --lower_frame_range=$LOWER_FRAME_RANGE
